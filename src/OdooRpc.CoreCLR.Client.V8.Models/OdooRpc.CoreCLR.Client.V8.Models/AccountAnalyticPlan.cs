namespace OdooRpc.CoreCLR.Client.V8.Models;

public partial class AccountAnalyticPlan
{
    public virtual string name { get; set; }
    public virtual string description { get; set; }
    public virtual object parent_id { get; set; }
    public virtual string parent_path { get; set; }
    public virtual object root_id { get; set; }
    public virtual long[] children_ids { get; set; }
    public virtual long children_count { get; set; }
    public virtual string complete_name { get; set; }
    public virtual long[] account_ids { get; set; }
    public virtual long account_count { get; set; }
    public virtual long all_account_count { get; set; }
    public virtual long color { get; set; }
    public virtual long sequence { get; set; }
    public virtual string default_applicability { get; set; }
    public virtual long[] applicability_ids { get; set; }
    public virtual long id { get; set; }
    public virtual string display_name { get; set; }
    public virtual object create_uid { get; set; }
    public virtual string create_date { get; set; }
    public virtual object write_uid { get; set; }
    public virtual string write_date { get; set; }
}